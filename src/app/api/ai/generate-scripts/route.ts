...
function mockScript(product: ProductInput, ambassadriceName: string) {
...
Story 5 : "Lien en bio pour commander${product.price ? ` — prix : ${product.price} €` : ''} 💕 Plus jamais ce problème !"`,
...
💡 ASTUCE AUTHENTICITÉ : Filme en une seule prise si possible — les gens sentent le vrai.`,
    hashtags: `#beauté #temoignage #transformation #routinebeauté #lmdelesthetique #martinique #antilles #conseilbeauté #beautyinfluencer #avant #après`,
  };
}

async function generateWithClaude(product: ProductInput, ambassadriceName: string, ambassadriceGrade: string, ambassadriceFollowers: number, apiKey: string) {
  const problemCtx = detectProblemContext(product.name);
...
Génère exactement ce JSON (sans markdown, juste le JSON pur) :
{
  ...
  "hashtags": "#hashtags dont #temoignage #transformation #lmdelesthetique #martinique et hashtags problème spécifique"
}`;
  ...
  for (const product of products) {
    try {
      scripts[product.id] = hasClaude
        ? await generateWithClaude(product, ambassadriceName, ambassadriceGrade, ambassadriceFollowers, apiKey)
        : mockScript(product, ambassadriceName);
    } catch (e: any) {
      console.error('[ai/generate-scripts] product error:', product.id, e.message);
      scripts[product.id] = mockScript(product, ambassadriceName);
    }
  }
